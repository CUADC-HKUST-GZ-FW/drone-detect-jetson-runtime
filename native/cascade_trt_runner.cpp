...
class TrtEngine {
 public:
  TrtEngine(const std::string& path, Logger& logger) {
    ...
    if (input_name_.empty() || output_host_.empty()) throw std::runtime_error("missing TRT input/output");
  }
...
struct Slot {
  ...
  float* stage1_host{};
  double read_ms{};
  double stage1_pre_ms{};
};

static cv::VideoCapture make_square_cap(const std::string& video_path, int target, int content_w, int content_h) {
...
static void square_bgr_to_nchw(cv::Mat& frame, float* dst, int target, int content_h) {
  const int pad_y = (target - content_h) / 2;
  if (pad_y > 0) {
...
static cv::Rect select_top_box(const float* out, size_t count, int size, float conf, bool& found, float& score_out) {
  ...
  const size_t dets = count / 6;
  for (size_t i = 0; i < dets; ++i) {
    const float* d = out + i * 6;
    float score = d[4];
    if (score < conf || score <= score_out) continue;
    int x1 = std::clamp(static_cast<int>(std::floor(d[0])), 0, size - 1);
    int y1 = std::clamp(static_cast<int>(std::floor(d[1])), 0, size - 1);
    int x2 = std::clamp(static_cast<int>(std::ceil(d[2])), 0, size - 1);
    int y2 = std::clamp(static_cast<int>(std::ceil(d[3])), 0, size - 1);
...
int main(int argc, char** argv) {
  if (argc < 5) {
    std::cerr << "usage: " << argv[0] << " <stage1.raw.engine> <stage2.raw.engine> <video.mp4> <stage1_size> [stage2_size=416] [warmup=5] [measure=60] [slots=4]\n";
    return 1;
  }
  std::string stage1_path = argv[1];
  std::string stage2_path = argv[2];
  std::string video_path = argv[3];
  int stage1_size = std::atoi(argv[4]);
  int stage2_size = argc > 5 ? std::atoi(argv[5]) : 416;
  double warmup_sec = argc > 6 ? std::atof(argv[6]) : 5.0;
  double measure_sec = argc > 7 ? std::atof(argv[7]) : 60.0;
  int slots_n = argc > 8 ? std::atoi(argv[8]) : 4;
  int content_w = stage1_size;
  int content_h = static_cast<int>(std::round(stage1_size * 9.0 / 16.0));

  Logger logger;
  TrtEngine stage1(stage1_path, logger);
  TrtEngine stage2(stage2_path, logger);
  if (stage1.input_w() != stage1_size || stage2.input_w() != stage2_size) {
    std::cerr << "engine input mismatch: stage1=" << stage1.input_w() << " stage2=" << stage2.input_w() << "\n";
    return 2;
  }
  cudaStream_t stream{};
  check_cuda(cudaStreamCreate(&stream), "stream");
  float* stage2_host{};
  check_cuda(cudaHostAlloc(reinterpret_cast<void**>(&stage2_host), stage2.input_bytes(), cudaHostAllocDefault), "stage2 host");

  std::vector<Slot> slots(slots_n);
  for (auto& s : slots) {
    s.frame_bgr.create(stage1_size, stage1_size, CV_8UC3);
    check_cuda(cudaHostAlloc(reinterpret_cast<void**>(&s.stage1_host), stage1.input_bytes(), cudaHostAllocDefault), "stage1 host");
  }

  auto run_phase = [&](double seconds, bool measure) {
    Queue free_q, ready_q;
    for (int i = 0; i < slots_n; ++i) free_q.push(i);
    std::atomic<bool> done{false};
    std::atomic<int> frames{0};
    int fallbacks = 0;
    std::vector<double> read_ms, pre1_ms, infer1_ms, crop_ms, infer2_ms, active_ms, scores, roi_area;
    auto start = Clock::now();
    auto end = start + std::chrono::duration_cast<Clock::duration>(std::chrono::duration<double>(seconds));

    std::thread producer([&] {
      cv::VideoCapture cap = make_square_cap(video_path, stage1_size, content_w, content_h);
      cv::Mat frame;
      while (Clock::now() < end) {
        int idx = -1;
        ...
        if (!cap.read(frame)) {
          cap.release();
          cap = make_square_cap(video_path, stage1_size, content_w, content_h);
          if (!cap.read(frame)) {
            free_q.push(idx);
            break;
          }
        ...
        auto r1 = Clock::now();
        if (frame.cols != stage1_size || frame.rows != stage1_size) {
          std::cerr << "strict stage1 frame size violation: " << frame.cols << "x" << frame.rows << "\n";
          std::exit(5);
        }
        frame.copyTo(slots[idx].frame_bgr);
        square_bgr_to_nchw(slots[idx].frame_bgr, slots[idx].stage1_host, stage1_size, content_h);
        auto p1 = Clock::now();
        slots[idx].read_ms = std::chrono::duration<double, std::milli>(r1 - r0).count();
        slots[idx].stage1_pre_ms = std::chrono::duration<double, std::milli>(p1 - r1).count();
        ready_q.push(idx);
      }
      done = true;
      ready_q.stop();
    ...
    while (true) {
      ...
      auto a0 = Clock::now();
      double t1 = stage1.run(stream, slots[idx].stage1_host);
      bool found = false;
      float score = 0.0f;
      cv::Rect box = select_top_box(stage1.output0(), stage1.output0_count(), stage1_size, 0.25f, found, score);
      if (!found) ++fallbacks;
      auto c0 = Clock::now();
      cv::Mat roi = slots[idx].frame_bgr(box);
      crop_pre.run(roi, stage2_host);
      auto c1 = Clock::now();
      double t2 = stage2.run(stream, stage2_host);
      auto a1 = Clock::now();
      if (measure) {
        ++frames;
        read_ms.push_back(slots[idx].read_ms);
        pre1_ms.push_back(slots[idx].stage1_pre_ms);
        infer1_ms.push_back(t1);
        crop_ms.push_back(std::chrono::duration<double, std::milli>(c1 - c0).count());
        infer2_ms.push_back(t2);
        active_ms.push_back(std::chrono::duration<double, std::milli>(a1 - a0).count() + slots[idx].read_ms + slots[idx].stage1_pre_ms);
        scores.push_back(score);
        roi_area.push_back(static_cast<double>(box.area()) / (stage1_size * stage1_size));
      }
      free_q.push(idx);
    }
    free_q.stop();
  ...
  std::cout << "{"
            << "\"mode\":\"cascade_stage1_location_stage2_type\","
            << "\"stage1_size\":" << stage1_size << ","
            << "\"stage2_requested_size\":400,"
            << "\"stage2_actual_size\":" << stage2_size << ","
            << "\"strict_stage1_input_w\":" << stage1_size << ","
            << "\"strict_stage1_input_h\":" << stage1_size << ","
            << "\"frames\":" << frames << ","
            << "\"wall_sec\":" << wall_sec << ","
  ...
  cudaFreeHost(stage2_host);
  for (auto& s : slots) cudaFreeHost(s.stage1_host);
