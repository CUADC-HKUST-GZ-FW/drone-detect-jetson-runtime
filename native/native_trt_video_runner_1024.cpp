...
int main(int argc, char** argv) {
  if (argc < 3) {
    std::cerr << "usage: " << argv[0] << " <raw.engine> <video.mp4> [warmup_sec=5] [measure_sec=60] [target=800]\n";
    return 1;
  }
  ...
  int target = argc > 5 ? std::atoi(argv[5]) : 800;
  ...
  auto run_until = [&](double seconds, bool measure) {
    ...
    while (Clock::now() < end) {
      ...
      auto r1 = Clock::now();
      preprocess_bgr_to_nchw(frame, static_cast<float*>(b.input_host), target);
      auto p1 = Clock::now();
